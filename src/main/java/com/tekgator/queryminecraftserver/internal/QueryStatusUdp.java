...
public class QueryStatusUdp extends QueryStatusBase {
    ...
    private static final byte[] MAGIC_BYTES_FULLSTAT = { (byte) 0x73, (byte) 0x70, (byte) 0x6C, (byte) 0x69,
            (byte) 0x74, (byte) 0x6E, (byte) 0x75, (byte) 0x6D, (byte) 0x00, (byte) 0x80, (byte) 0x00 };
    ...
    private byte[] requestData(boolean isHandshake) throws IOException {
    ...
    private byte[] receiveMessage(boolean isHandshake) throws IOException {
        ...
        if (sessionIdRcv != this.sessionId) {
            throw new StreamCorruptedException(String.format("Session ID received '%d' is different from sent '%d'", sessionIdRcv, this.sessionId));
        }

        if (msgTypeRcv != getMessageByte(isHandshake)) {
            throw new StreamCorruptedException(String.format("Message type received '%d' is different from sent '%d'", msgTypeRcv, getMessageByte(isHandshake)));
        }
        ...
        if (!isHandshake && protocol == Protocol.UDP_FULL) {
            ...
            if (!Arrays.equals(fullStatCheck, MAGIC_BYTES_FULLSTAT)) {
                throw new StreamCorruptedException("Expected data stream is invalid, missing magic bytes for full stat!");
            }
        }
